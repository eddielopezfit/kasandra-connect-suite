  ...
  try {
    ...
    if (GHL_WEBHOOK_URL && (leadId || body.session_id)) {
      const ghlPayload = {
        ...
        tools_completed: body.tools_completed?.join(",") || "",
        guides_read_count: body.guides_read_count || 0,
        journey_state: body.journey_state || "explore",
        military_flag: body.military_flag || false,
        inherited_home: body.inherited_home || false,
        estimated_value: body.estimated_value || 0,
        estimated_budget: body.estimated_budget || 0,
        calculator_advantage: body.calculator_advantage || "",
        seller_decision_path: body.seller_decision_path || "",
        dossier_summary: dossierSummary,
        custom_fields: ghlFields,
        tags: [
          "booking_started",
          body.military_flag ? "military_buyer" : null,
          body.inherited_home ? "inherited_property" : null,
          body.language === "es" ? "language_es" : null,
          body.readiness_score && body.readiness_score >= 75 ? "high_readiness" : null,
        ].filter(Boolean),
      };
