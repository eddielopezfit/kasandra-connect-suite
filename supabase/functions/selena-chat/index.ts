...
// Persists across hot Deno isolate invocations. 1h TTL market data, 24h neighborhoods.
...
// Strips intent-irrelevant KB sections to reduce token count ~20-35%.
...
function buildSystemPrompt(
  ...
  // Strip seller-specific KB when user is a pure buyer or exploring
  if (!isSeller) {
    if (language === 'es') {
  ...
  // Strip buyer-specific KB when user is a pure seller or exploring
  if (!isBuyer) {
    if (language === 'es') {
  ...
  // Strip platform context for pure explore with no tool engagement
