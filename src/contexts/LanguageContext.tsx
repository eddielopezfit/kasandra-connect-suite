 ...
 *
 * LANGUAGE LOCK:
 * - UI language is a single source of truth from LanguageContext
 * - No drawer-local uiLanguage state
 * - Toggle updates global language only
 */
...
export function SelenaChatDrawer() {
  ...
  // Global language toggle handler (single source of truth)
  const handleLanguageToggle = useCallback(() => {
    const newLang = language === "en" ? "es" : "en";
    setLanguage(newLang);
    logEvent("ui_language_toggle", { from: language, to: newLang });
  ...
  const MessagesArea = () => (
    <ScrollArea className="flex-1 p-4" ref={scrollRef} onClick={handleMessagesAreaClick}>
      <div className="space-y-4 pb-2">
        {messages.map((message) => (
          <MessageBubble key={message.id} message={message} onActionClick={handleActionClick} />
        ))}
    ...
    return (
      <>
        ...
        >
          ...
          <span className="text-sm font-medium">{t("Selena is active", "Selena está activa")}</span>
  ...
  return (
    <>
      <Sheet open={isOpen} onOpenChange={(open) => !open && closeChat()}>
        <SheetContent side="right" className="w-[400px] max-w-[35vw] min-w-[320px] p-0 flex flex-col h-full">
