...
export const ctaRegistry: CTATestEntry[] = [
  ...
  // ── ActionSpec Chips (Chat Drawer) ──
  {
    ...
    expectedTarget: '/book',
    automatable: false,
    manualSteps: 'Sell flow → ASAP → Phase 3 chips appear → Click "Talk with Kasandra". Verify: route=/book, NO selena-chat POST, NO user message appended.',
    priority: 17,
  ...
  },
  {
    ...
    expectedTarget: '/cash-offer-options',
    automatable: false,
    manualSteps: 'Sell flow → ASAP → Phase 3 chips appear → Click "Estimate my net proceeds". Verify: route=/cash-offer-options, NO selena-chat POST, NO user message appended.',
    priority: 18,
    notes: 'ActionSpec chip — must NOT trigger edge function or append message.',
  },
